...
namespace QuickChat
{
	...
	internal class ShortcutHandler
	{
		...
		{
			QuickChatUseAdvanced = Plugin.ConfigF.Bind("General", "Use Advanced Shortcut Definition", false, "Allows user to define shortcuts with one Text Field (Only one restart required).");
			QuickChatPrefixOnChat = Plugin.ConfigF.Bind("General", "Put Prefix on Chat", false, "Automatically your desired prefix when you open chat.");
			QuickChatCaseSensitive = Plugin.ConfigF.Bind("Shortcuts", "Is Case Sensitive?", false, "Are the shortcuts case sensitive? (Requires shortcut to be UPPERCASE or lowercase depending on definition of shortcut).");
			QuickChatPrefix = Plugin.ConfigF.Bind("Shortcuts", "Chat Prefix", "/", "The prefix to use before a shortcut (say the prefix was \"/\": [/SHORTCUT_NAME => MESSAGE].");

			var quickChatUseAdvancedField = new BoolCheckBoxConfigItem(QuickChatUseAdvanced, requiresRestart: true);
			...
			if (!UseAdvanced)
			{
				QuickChatSimpleShortcutsNumber = Plugin.ConfigF.Bind("Shortcuts", "Simple Shortcuts Amount", 4, "Amount of simple shortcuts (REQUIRES RESTART TO UPDATE).");
				...
				for (int i = 0; i < SimpleShortcutsNum; i++)
				{
					...
					var simpleName = Plugin.ConfigF.Bind($"Simple Shortcut {displayI}", "Name", $"Name{displayI}", "The Name of the Shortcut that will be changed to the Result on enter.");
					...
					var simpleResult = Plugin.ConfigF.Bind($"Simple Shortcut {displayI}", "Result", $"Result{displayI}", "The Result that will be shown after entering the Shortcut.");
			...
			{
				QuickChatShortcutsAdvanced = Plugin.ConfigF.Bind("Shortcuts", "Advanced Chat Shortcuts", "a:My Balls", "Shortcuts in format of [SHORTCUT_NAME : MESSAGE, SHORTCUT_NAME2 : MESSAGE2] without the [].");
